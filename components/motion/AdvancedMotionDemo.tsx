    ...
    return (
      ...
      case 'morphing':
        return (
          <div className="relative h-96 flex items-center justify-center">
            ...
            >
              <motion.div
                className="w-32 h-32 bg-gradient-to-br from-brand to-ok"
                animate={{
                  clipPath: [
      ...
      case 'magnetic':
        return (
          <div className="relative h-96">
            {physicsState.elements.slice(0, 4).map((element, index) => (
              <motion.div
                key={element.id}
                className="absolute w-16 h-16 rounded-full flex items-center justify-center text-2xl cursor-pointer"
                style={{
                  backgroundColor: element.color + '20',
                  border: `2px solid ${element.color}`,
                  left: element.x,
                  top: element.y,
                  x: useTransform(mouseX, [element.x - 100, element.x + 100], [-20, 20]),
                  y: useTransform(mouseY, [element.y - 100, element.y + 100], [-20, 20])
                }}
      ...
      default:
        return (
          <div className="relative h-96">
            <AnimatePresence>
              {physicsState.elements.map((element) => (
                <motion.div
                  key={element.id}
                  className="absolute cursor-pointer flex items-center justify-center text-white font-bold rounded-lg shadow-lg"
                  style={{
                    width: element.size,
                    height: element.size,
                    ...
                    fontSize: Math.max(element.size / 3, 12)
                  }}
                  initial={{ scale: 0, opacity: 0 }}
                  animate={{ 
                    scale: physicsState.selectedElement === element.id ? 1.2 : 1,
                    opacity: 1,
                    rotate: element.isActive ? [0, 360] : 0
                  }}
